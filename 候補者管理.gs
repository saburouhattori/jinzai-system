  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      if (String(data[i][0]) === searchId) {
        ...
        res.birthday = getVal('生年月日');
        res.gender = getVal('性別');
        res.spouse = getVal('配偶者');
        res.height = getVal('身長');
        res.weight = getVal('体重');
        res.address = getVal('現住所');
        res.birthplace = getVal('住所（出身地）');
        res.email = getVal('メールアドレス');
        res.school = getVal('所属日本語学校');
        res.eduSchool = getVal('学歴＞学校名');
        res.eduDept = getVal('学歴＞学部・学科・専攻');
        res.eduStatus = getVal('学歴＞状況');
        res.eduStart = getVal('学歴＞入学年月');
        res.eduEnd = getVal('学歴＞卒業/中退年月');
        res.eduNote = getVal('学歴＞補足');
        res.expPeriod1 = getVal('職歴①＞期間');
        res.expContent1 = getVal('職歴①＞内容');
        res.expPeriod2 = getVal('職歴②＞期間');
        res.expContent2 = getVal('職歴②＞内容');
        res.expPeriod3 = getVal('職歴③＞期間');
        res.expContent3 = getVal('職歴③＞内容');
        res.jlptLevel = getVal('特定技能要件＞JLPTレベル');
        res.jlptDate = getVal('特定技能要件＞JLPT取得年月');
        res.jftLevel = getVal('特定技能要件＞JFTBasicレベル');
        res.jftDate = getVal('特定技能要件＞JFT取得年月');
        res.kaigoSkill = getVal('特定技能要件＞介護技能評価試験');
        res.kaigoSkillDate = getVal('特定技能要件＞介護技能取得年月');
        res.kaigoLang = getVal('特定技能要件＞介護日本語評価試験');
        res.kaigoLangDate = getVal('特定技能要件＞介護日本語取得年月');
        res.otherJapanese = getVal('その他の日本語能力試験');
        res.otherJapaneseDate = getVal('取得年月');
        ...
        res.addressDetail = getVal('住所詳細');
        res.passportNum = getVal('パスポート番号');
        res.passportExp = getVal('パスポート有効期限');
        res.job = getVal('職業');
        res.traineeExp = getVal('技能実習の経験の有無');
        res.traineeCert = getVal('技能実習修了書の有無');
        res.crime = getVal('犯罪歴の有無');
        res.applyCount = getVal('在留資格交付申請の回数');
        res.rejectCount = getVal('不許可となった在留資格交付申請の回数');
        res.overseasExp = getVal('海外への出入国歴の有無');
        res.travelCount = getVal('出入国の回数');
        res.lastInDate = getVal('直近の入国日');
        res.lastOutDate = getVal('直近の出国日');
        res.relName2 = getVal('日本在住の親族情報親族の名前');
        res.relRelation2 = getVal('日本在住の親族情報続柄');
        res.relBirth2 = getVal('日本在住の親族情報親族の生年月日');
        res.relCountry2 = getVal('日本在住の親族情報親族の国籍・地域');
        res.relLive2 = getVal('日本在住の親族情報親族との同居予定の有無');
        res.relWork2 = getVal('日本在住の親族情報親族の勤務先・通学先');
        res.relCard2 = getVal('日本在住の親族情報親族の在留カード番号');
        res.memo = getVal('備考・メモ');
